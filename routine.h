...
// extern
extern FORCEINLINE SIZE_T _r_str_getlength (
	_In_ LPCWSTR string
);

extern FORCEINLINE SIZE_T _r_str_getlength_ex (
	_In_ LPCWSTR string,
	_In_ SIZE_T max_count
);

extern FORCEINLINE SIZE_T _r_str_getlength2 (
	_In_ PR_STRING string
);

extern FORCEINLINE SIZE_T _r_str_getlength3 (
	_In_ PR_STRINGREF string
);

extern FORCEINLINE SIZE_T _r_str_getbytelength (
	_In_ LPCSTR string
);

extern FORCEINLINE SIZE_T _r_str_getbytelength_ex (
	_In_ LPCSTR string,
	_In_ SIZE_T max_count
);

extern FORCEINLINE SIZE_T _r_str_getbytelength2 (
	_In_ PR_BYTE string
);

extern FORCEINLINE SIZE_T _r_str_getbytelength3 (
...
FORCEINLINE VOID FASTCALL _r_event_intialize (
...
_Ret_maybenull_
FORCEINLINE PVOID _r_obj_referencesafe (
...
FORCEINLINE VOID _r_obj_dereference (
	_In_ PVOID object_body
)
{
...
FORCEINLINE VOID _r_obj_dereferencelist (
...
FORCEINLINE PR_BYTE _r_obj_createbyte (
	_In_ LPSTR string
)
{
	return _r_obj_createbyte_ex (
		string,
		_r_str_getbytelength (string)
	);
}
...
)
{
	return _r_obj_createstring_ex (
		string,
		_r_str_getlength (string) * sizeof (WCHAR)
	);
}
...
FORCEINLINE PR_STRING _r_obj_createstring4 (
...
FORCEINLINE BOOLEAN _r_obj_isstringnullterminated (
	_In_ PR_STRINGREF string
)
{
...
FORCEINLINE VOID _r_obj_skipstringlength (
...
FORCEINLINE VOID _r_obj_writestringnullterminator (
	_In_ PR_STRING string
)
{
...
)
{
	_r_obj_initializestringref_ex (
		string,
		(LPWSTR)buffer,
		_r_str_getlength (buffer) * sizeof (WCHAR)
	);
}
...
FORCEINLINE VOID _r_obj_initializestringbuilder (
	_Out_ PR_STRINGBUILDER builder
)
{
	_r_obj_initializestringbuilder_ex (
		builder,
		256 * sizeof (WCHAR)
	);
}
...
FORCEINLINE VOID _r_obj_insertstringbuilderformat (
	_Inout_ PR_STRINGBUILDER builder,
	_In_ SIZE_T index,
	_In_ _Printf_format_string_ LPCWSTR format,
	...
)
{
	...
	_r_obj_insertstringbuilderformat_v (
		builder,
		index,
		format,
		arg_ptr
...
{
	return _r_obj_removehashtableitem (
		hashtable,
		hash_code
	);
}
...
{
	_r_console_writestring_ex (
		string,
		(ULONG)_r_str_getlength (string)
	);
}
...
)
{
	_r_console_writestring_ex (
		string->buffer,
		(ULONG)_r_str_getlength2 (string)
	);
}
...
)
{
	_r_console_writestring_ex (
		string->buffer,
		(ULONG)_r_str_getlength3 (string)
	);
}
...
FORCEINLINE LONG _r_calc_clamp32 (
	_In_ LONG value,
	_In_ LONG min_value,
	_In_ LONG max_value
)
...
FORCEINLINE VOID _r_calc_millisecondstolargeinteger (
...
FORCEINLINE ULONG _r_calc_multipledivide (
	_In_ ULONG number,
...
FORCEINLINE LONG _r_calc_multipledividesigned (
	_In_ LONG number,
	_In_ ULONG numerator,
	_In_ ULONG denominator
)
{
	...
	{
		return -(LONG)_r_calc_multipledivide (
			-number,
			numerator,
			denominator
		);
...
FORCEINLINE LONG64 _r_fs_getsize (
	_In_ HANDLE hfile
)
{
	LARGE_INTEGER size;
...
FORCEINLINE ULONG _r_str_gethash2 (
	_In_ PR_STRING string,
	_In_ BOOLEAN is_ignorecase
)
{
	return _r_str_x65599 (
		&string->sr,
		is_ignorecase
	);
}

FORCEINLINE ULONG _r_str_gethash3 (
	_In_ PR_STRINGREF string,
	_In_ BOOLEAN is_ignorecase
)
{
...
FORCEINLINE INT _r_str_compare (
...
FORCEINLINE INT _r_str_compare_length (
	_In_ LPCWSTR string1,
	_In_ LPCWSTR string2,
	_In_ SIZE_T length
)
...
_Success_ (return)
FORCEINLINE BOOLEAN _r_str_frominteger (
	_Out_writes_ (buffer_size) LPWSTR buffer,
	_In_ SIZE_T buffer_size,
	_In_ INT value
)
{
	return _r_str_printf (
		buffer,
		buffer_size,
		L"%" TEXT (PRIi32),
		value
...
_Success_ (return)
FORCEINLINE BOOLEAN _r_str_fromuinteger (
	_Out_writes_ (buffer_size) LPWSTR buffer,
	_In_ SIZE_T buffer_size,
	_In_ UINT value
)
...
FORCEINLINE SIZE_T _r_str_getlength (
	_In_ LPCWSTR string
)
{
	return _r_str_getlength_ex (
		string,
		PR_SIZE_MAX_STRING_LENGTH
	);
}

FORCEINLINE SIZE_T _r_str_getlength2 (
	_In_ PR_STRING string
)
{
	assert (!(string->length & 0x01));
...
FORCEINLINE SIZE_T _r_str_getlength3 (
	_In_ PR_STRINGREF string
)
{
	assert (!(string->length & 0x01));
...
FORCEINLINE SIZE_T _r_str_getlength4 (
	_In_ PUNICODE_STRING string
)
{
	assert (!(string->Length & 0x01));
...
FORCEINLINE SIZE_T _r_str_getbytelength_ex (
	_In_ LPCSTR string,
	_In_ SIZE_T max_count
)
{
...
FORCEINLINE SIZE_T _r_str_getbytelength (
	_In_ LPCSTR string
)
{
	return _r_str_getbytelength_ex (
		string,
		PR_SIZE_MAX_STRING_LENGTH
	);
}

FORCEINLINE SIZE_T _r_str_getbytelength2 (
	_In_ PR_BYTE string
)
{
...
FORCEINLINE SIZE_T _r_str_getbytelength3 (
	_In_ PR_BYTEREF string
)
{
	return string->length;
...
FORCEINLINE VOID _r_sys_closehandle (
	_Inout_ PVOID_PTR handle_address
)
{
	HANDLE handle;
...
FORCEINLINE ULONG _r_sys_createprocess (
	_In_opt_ LPCWSTR file_name,
	_In_opt_ LPCWSTR command_line,
	_In_opt_ LPCWSTR current_directory
)
...
FORCEINLINE VOID _r_dc_fixcontrolfont (
	_In_ HDC hdc,
	_In_ HWND hwnd,
	_In_ INT ctrl_id
)
...
FORCEINLINE VOID _r_dc_fixwindowfont (
	_In_ HDC hdc,
	_In_ HWND hwnd
)
{
...
{
	return _r_calc_multipledividesigned (
		number,
		dpi_value,
...
FORCEINLINE VOID _r_wnd_adjustrectangletobounds (
	_Inout_ PR_RECTANGLE rectangle,
...
FORCEINLINE VOID _r_wnd_centerwindowrect (
...
FORCEINLINE VOID _r_wnd_setrectangle (
	_Out_ PR_RECTANGLE rectangle,
	_In_ LONG left,
	_In_ LONG top,
	_In_ LONG width,
...
)
{
	SetWindowPos (
		...
		0,
		0,
		SWP_NOMOVE | SWP_NOSIZE | SWP_NOREDRAW | SWP_NOACTIVATE | SWP_NOOWNERZORDER
	);
}
...
FORCEINLINE ULONG _r_math_getrandomrange (
...
FORCEINLINE ULONG _r_math_hashinteger32 (
	_In_ ULONG value
)
{
	// Java style.
...
FORCEINLINE ULONG _r_math_hashinteger64 (
	_In_ ULONG64 value
)
{
...
_Success_ (return == S_OK)
FORCEINLINE HRESULT _r_xml_parsefile (
	_Inout_ PR_XML_LIBRARY xml_library,
	_In_ LPCWSTR file_path
)
{
...
_Success_ (return == S_OK)
FORCEINLINE HRESULT _r_xml_parsestring (
	_Inout_ PR_XML_LIBRARY xml_library,
	_In_ LPCVOID buffer,
	_In_ ULONG buffer_length
)
...
VOID _r_ctrl_setbuttonmargins (
	_In_ HWND hwnd, _In_ INT ctrl_id,
	_In_ LONG dpi_value
);
...
)
{
	EnableMenuItem (
		hmenu,
		item_id,
		position_flag | (is_enable ? MF_ENABLED : MF_DISABLED | MF_GRAYED)
	);
}
...
{
	return (ULONG)SendDlgItemMessage (
		hwnd,
		ctrl_id,
